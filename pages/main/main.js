...
Page({
  ...
  getShopData: function() {
    ...
    app.getApiData(params, "v2-5-7/course/shop-list", function success(result) {
      that.setData({
        ...
        recentData: result.data.obj.recent_date
      })
      that.stopLoading()
    })
  },
  ...
  recentCourseBtn:function(e){
    var that = this
    for (var i = 0; i < that.data.allWeek.length; i++ ){
      if (that.data.allWeek[i].dates == that.data.recentData){
        this.setData({
          weekState: i,
          day: that.data.recentData , 
        }, function () {
          that.getShopData()
        })
      }
